...
func _ready():
	...
	__wait_for_resources = false

# Called every frame. 'delta' is the elapsed time since the previous frame.
func _process(delta):
	if __wait_for_resources == true:
		if __got_bugs_found and __got_bug_issues:
			execute_sprint_end()


# Start process of executing the end of sprint. Query Game for bugs found
# and bug issues.
func start_sprint_end():
	__got_bugs_found = false
	__got_bug_issues = false
	emit_signal("request_bug_issues")
	emit_signal("request_quality_cards", __issues_done_this_sprint)

# Execute the end of sprint. It will be done after receive resources.
